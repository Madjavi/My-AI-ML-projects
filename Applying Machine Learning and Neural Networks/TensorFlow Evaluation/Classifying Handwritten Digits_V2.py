...
class NN_model_trainer:
    ...
    hidden_weights = tf.Variable(tf.compat.v1.truncated_normal([hidden_neurons, hidden_neurons_2]))
    hidden_biases = tf.Variable(tf.zeros([hidden_neurons_2]))

    hidden_weights_2 = tf.Variable(tf.compat.v1.truncated_normal([hidden_neurons_2, 10]))
    hidden_biases_2 = tf.Variable(tf.zeros([10]))

    input_layer = tf.matmul(input_images, input_weights) + input_biases
    hidden_layer = tf.nn.relu(tf.matmul(input_layer, hidden_weights) + hidden_biases)
    digit_weights = tf.matmul(hidden_layer, hidden_weights_2) + hidden_biases_2
...
# This class trains the model applying different learning rates to measure how affects the overall accuracy.
class training_rate_changer:
    rates_of_learning = [0.1, 0.01, 0.001, 0.0001]

    for learner in rates_of_learning:
        optimizer_2 = tf.compat.v1.train.GradientDescentOptimizer(learner).minimize(NN_model_trainer.loss_function)

        tf.compat.v1.global_variables_initializer().run()

        for steps in range(NN_model_trainer.EPOCH):
            ...
            results = f"\nTraining epoch ________________________________{str(steps + 1)}\n"
            ...
            for miscl_data_2 in range(5):
                plt.subplot(1, 5, miscl_data_2 + 1)
                label = load_pre_pross.y_test[misclassified_idx[miscl_data_2]].argmax(axis=0)
                image = load_pre_pross.x_test[misclassified_idx[miscl_data_2]].reshape([28, 28])
                plt.title("misclassified image\n" + 'Label: {}\nPredicted: {}'.format(label,
                           NN_model_trainer.digit_weights.eval(feed_dict={NN_model_trainer.input_images:
                           load_pre_pross.x_test[misclassified_idx[miscl_data_2]].reshape(1, 784)})[0].argmax()))
            ...
            result_2 = f"Learning Rate: {learner}\n"
